 ...
 * Raises an error if a mismatch (e.g., CPU vs. XPU, or xpu:0 vs. xpu:1) is
 * detected.
 */
...
static bool lazy_registration_and_redispatch(
    const c10::OperatorHandle& op,
    torch::jit::Stack* stack) {
  auto register_func = [](torch::Library& m) -> void {
...
TORCH_LIBRARY_IMPL(_, XPU, m) {
  ...
  if (!enable_xpu_fallback || std::stoi(enable_xpu_fallback) == 0) {
    m.fallback(torch::CppFunction::makeFromBoxedFunction<
               &xpu_lazy_registration_or_error_fallback>());
  } else {
...
TORCH_LIBRARY_IMPL(aten, XPU, m) {
  ...
  while (std::getline(iss, op_name, ',')) {
    TORCH_WARN(
        "The operator '",
        op_name,
        "' registered to be forced to fallback to CPU.");
    m.impl(
        op_name.c_str(),
...
/*
 * These ops are not supported via XPU backend currently, and we fallback to run
 * on CPU.
 */
TORCH_LIBRARY_IMPL(aten, XPU, m) {
